...
import { customerAddressAPI, addressAPI, promotionAPI, orderAPI, momoPaymentAPI } from '../services/api';
...
const CheckoutPage = () => {
  ...
  useEffect(() => {
    ...
    if (!isOrderingRef.current && (!cart || !cart.items || cart.items.length === 0)) {
      navigate('/cart');
      return;
    }
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (paymentMethod === PAYMENT_METHODS.MOMO) {
           try {
               const momoRes = await momoPaymentAPI.createPayment({ 
                   orderId: newOrderId 
               });
               ...
               if (momoRes.data && momoRes.data.payUrl) {
                   ...
                   setMessage({ type: 'error', text: 'Không lấy được link thanh toán MoMo.' });
               }
           } catch (momoError) {
               isOrderingRef.current = false;
               setMessage({ type: 'error', text: 'Lỗi kết nối cổng thanh toán MoMo.' });
           }
        } else {
            await clearCart();
            navigate(`/payment-result?resultCode=COD&orderId=${newOrderId}&message=Đặt hàng thành công`);
        }
      } else {
        setMessage({ type: 'error', text: response.data.message || MESSAGES.ERROR.GENERIC });
      }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <div className="space-y-3">
                <label className={`flex items-center p-4 border-2 rounded-lg cursor-pointer transition-colors ${paymentMethod === PAYMENT_METHODS.COD ? 'border-primary-500 bg-primary-50' : 'hover:border-primary-500'}`}>
                  <input
                    type="radio"
                    name="paymentMethod"
                ...
                <label className={`flex items-center p-4 border-2 rounded-lg cursor-pointer transition-colors ${paymentMethod === PAYMENT_METHODS.MOMO ? 'border-[#a50064] bg-pink-50' : 'hover:border-[#a50064]'}`}>
                  <input
                    type="radio"
                    name="paymentMethod"
                    ...
                    onChange={(e) => setPaymentMethod(e.target.value)}
                    className="w-4 h-4 text-[#a50064] focus:ring-[#a50064]"
                  />
                  <div className="ml-3 flex items-center w-full">
                    <div className="flex-1">
                        <p className="font-medium text-gray-900">Ví điện tử MoMo</p>
                        <p className="text-sm text-gray-500">Quét mã QR qua ứng dụng MoMo</p>
                    </div>
                    <img src="https://upload.wikimedia.org/wikipedia/vi/f/fe/MoMo_Logo.png" alt="MoMo" className="h-8 w-8 object-contain ml-2" />
                  </div>
                </label>
              </div>
            </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-lg shadow p-6 sticky top-24">
              ...
              >
                {submitting ? 'Đang xử lý...' : (paymentMethod === PAYMENT_METHODS.MOMO ? 'Thanh toán MoMo' : 'Đặt hàng')}
              </button>
